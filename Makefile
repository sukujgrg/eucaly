...
release-notarize:
	...
	bash ./scripts/release-notarize-distribute.sh --notary-profile "$(NOTARY_PROFILE)" $(if $(TAG),--tag "$(TAG)") $(if $(BUILD_NUMBER),--build-number "$(BUILD_NUMBER)") $(if $(SKIP_VERSION_FILE_CHECK),--skip-version-file-check)
...
release-github:
	...
	bash ./scripts/release-notarize-distribute.sh --notary-profile "$(NOTARY_PROFILE)" --github $(if $(GH_REPO),--repo "$(GH_REPO)") $(if $(TAG),--tag "$(TAG)") $(if $(BUILD_NUMBER),--build-number "$(BUILD_NUMBER)") $(if $(NOTES_FILE),--notes "$(NOTES_FILE)") $(if $(SKIP_VERSION_FILE_CHECK),--skip-version-file-check)
